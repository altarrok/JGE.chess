...
public class Queen extends Piece {
    ...
    public Queen(Color color, Point initPos) {
        super(color, "QUEEN", initPos);
        moves = new Point[]{
                new Point(0, -1),
                new Point(1, 0),
                new Point(0, 1),
        ...
        this.tile = new ImageTile("/tiles/chess_sprites/" + (color == Color.BLACK ? "black" : "white") + "Queen.png", 50, 50);
    }
    ...
    public Point[] availMoves(Board b) {
        ...
        Piece currP;
        boolean[] doContinue = {true, true, true, true, true, true, true, true};
        int nextX, nextY;
        for (int i = 1; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                nextX = x + moves[j].x * i;
